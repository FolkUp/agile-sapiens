...
> **AGIL-190 Theme Correction Phase 2 in progress** (kickoff 2026-05-24): the 2026-05-19 "Literary Theme Correction" (AGIL-170) cleaned the book frame (homepage, about, cover, EPUB/PDF templates) but the chapter bodies 6-10 + afterword retained the AI overlay (16-125 ИИ mentions per chapter; Ch.0-5 were always Agile-faithful). Multi-session campaign. **Batches 1 + 2 + 3 + 4 SHIPPED**:
...
> - **EPUB 11.30 MB + PDF 3.84 MB** regenerated and synced to `static/downloads/` (SHA256 byte-match formats↔static verified).
> 
> Remaining: Ch.9 REPLACE author (Vinge → Стругацкие НИИЧАВО), Ch.10 REPLACE author (Gibson → Камю «Чума»), afterword REWRITE (synthesis + 11-archetype toolkit). Each chapter = separate session with hostile-review plan + result.
