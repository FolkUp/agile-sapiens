...
Last production deploy: PR #99 (AGIL-187 Batch C — removed the never-built English version; site is now RU-only: `[languages.en]` config, language switcher and EN content dropped, no `/en/` routes or hreflang), merged 2026-05-22; build verified RU-only (162 pages, flat sitemap, zero EN artifacts), render + axe spot-checked. Earlier the same day: PR #98 (Batch A accessibility — landmark structure + contrast). AGIL-187 audit fronts done: accessibility (Batch A) + EN-removal (Batch C); remaining: SEO meta/schema (Batch B) and the new cover (AGIL-185).  
