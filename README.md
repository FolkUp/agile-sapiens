...
Last production deploy: PR #101 (AGIL-187 Batch B — SEO meta/schema: homepage JSON-LD Organization→Book, twitter-card de-duplication, twitter:site / og:site_name / og:locale fixes, plus a pre-existing JSON-LD double-encoding bug fixed site-wide), merged 2026-05-22; prod-verified — 167/167 JSON-LD blocks parse as valid JSON. Same day earlier: PR #98 (Batch A accessibility — landmarks + contrast), PR #99 (Batch C — EN-removal). AGIL-187 audit fronts done: accessibility (A) + EN-removal (C) + SEO (B); remaining: the new cover (AGIL-185) and link-audit finish (in-page anchors).  
